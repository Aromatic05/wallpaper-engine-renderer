...
std::string BuildPersistentScript(std::string_view script_source) {
    ...
    wrapper
        ...
        << "  // engine.canvasSize and engine.screenResolution are seeded by the native host as plain "
           "{x,y} objects. Wallpaper Engine scripts call vector helpers such as divide() and add() on "
           "them (e.g. engine.canvasSize.divide(2)), so upgrade them to Vec2 instances after the Vec2 "
           "class is defined. Re-wrap every frame so native x/y updates are not lost.\n"
        << "  if (engine.canvasSize && typeof engine.canvasSize !== 'object') engine.canvasSize = "
           "{ x: Number(engine.canvasSize) || 0, y: Number(engine.canvasSize) || 0 };\n"
        << "  if (engine.screenResolution && typeof engine.screenResolution !== 'object') "
           "engine.screenResolution = { x: Number(engine.screenResolution) || 0, y: "
           "Number(engine.screenResolution) || 0 };\n"
        << "  if (typeof engine.canvasSize.divide !== 'function') engine.canvasSize = new "
           "Vec2(engine.canvasSize);\n"
        << "  if (typeof engine.screenResolution.divide !== 'function') engine.screenResolution = "
           "new Vec2(engine.screenResolution);\n"
